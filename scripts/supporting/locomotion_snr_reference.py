...
canonical locomotion analysis surface.
...
def load_grb006():
    ...
    trial_ts = pd.read_pickle(LOCAL_TRIAL_TS).reset_index(drop=True)
    first_stim = trial_ts["first_stim_ts"].to_numpy(dtype=float)
    first_stim = first_stim[np.isfinite(first_stim)]

    with LOCAL_SPIKE_TIMES.open("rb") as f:
        spike_df = pickle.load(f)
    unit_ids = spike_df["unit_id"].astype(int).tolist()
    spike_times = [
        np.asarray(t, dtype=float) / 30000.0 for t in spike_df["spike_times"].tolist()
    ]
