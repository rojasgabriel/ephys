...
overwrite_mode = (
    input("Do you want to run the script in overwrite mode? (y/n): ").strip().lower()
    == "y"
)

# Ask the user if they want to specify a particular mouse
specific_mouse = (
    input("Do you want to specify a particular mouse? (y/n): ").strip().lower() == "y"
)
...
if specific_mouse:
    ...
    specific_session = (
        input("Do you want to specify a particular session? (y/n): ").strip().lower()
        == "y"
    )
    if specific_session:
...
if not mice:
    print(f"No sessions found in {mice_paths}")
    sys.exit()
...
for mouse in mice:
    for session in tqdm(sessions[mouse], desc=f"Processing sessions for {mouse}"):
        ...
        if not auto_mode:
            process_session = (
                input(f"Do you want to process session {session}? (y/n): ")
                .strip()
                .lower()
                == "y"
            )
        ...
        try:
            ...
            if (
                not overwrite_mode
                and os.path.exists(spike_times_file)
                and os.path.exists(trial_ts_file)
            ):
                print(
                    f"Pre-processed files already exist for session {session}. Skipping processing."
                )
                continue
            ...
            ss = np.load(
                pjoin(kilosort_path, "spike_times.npy")
            )  # KS spikes (in samples)
            st = ss / srate  # conversion from spike samples to spike times

            print("Getting good units...")
            clu = spks.clusters.Clusters(
                folder=kilosort_path,
                get_waveforms=False,
                get_metrics=True,
                load_template_features=True,
            )
            good_units_mask, n_units = get_good_units(
                clusters_obj=clu, spike_clusters=sc
            )
            ...
            spike_times_per_unit = get_cluster_spike_times(
                spike_times=st, spike_clusters=sc, good_unit_ids=good_units_mask
            )
