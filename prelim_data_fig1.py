...
def get_balanced_trials(trial_ts, require_both_stim_types=False):
...
def plot_population_responses(trial_ts, spike_times_per_unit):
    ...
    ax1.legend([f"n={len(spike_times_per_unit)} neurons\n{min_trials} trials per outcome"], fontsize=14)
    ...
    plot_scatter_panel(ax4, move_stimulus_response, stat_stimulus_response,
                      ...
                      color=['green' if x > y else 'magenta' for x,y in zip(move_stimulus_response, stat_stimulus_response)])
...
def main():
    ...
    fig.savefig(fig_dir / 'first_stim_movement_vs_stationary.svg', format='svg')
    plt.close(fig)
