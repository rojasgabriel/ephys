...
figures_dir = "/Users/gabriel/lib/ephys/figures/adaptation"
...
# NOTE: these two variables are used for everything later on
...
melt_peak = peak_subtracted_df.melt(
    var_name="stimulus", value_name="response"
)  # massaging data to be in a nice format for seaborn plotting
...
# Trials where mice run to the left vs. the right will have visual stimuli moving in opposite directions across the visual field
# We want to make sure that we're in binocular zone V1 and that we can combine data from left and right trials
...
def extract_stims_by_side(trials_df, n_stims=4):
    """n_stims here is how many stims we're grabbing from the stationary ones

    Args:
        trials_df (pandas dataframe): pre_processed trial_ts
        n_stims (int, optional): number of stims to grab from the stationary list. Defaults to 4.

    Returns:
        ndarray: list of number of stims where every element is an array of stimulus timestamps
    """
...
def calculate_peak_responses(stim_list):
    """calculate a PSTH for a list of stims and grab the peak response for the pre-specified stimulus window

    Args:
        stim_list (ndarray): list of number of stims where every element is an array of stimulus timestamps

    Returns:
        list: list of number of stims where every element is the average peak response for every unit for that stimulus
    """
    peak_responses = []
    for stim_times in stim_list:
        ...
        stimulus_window_bool = (tb[:-1] >= stim_window_start) & (
            tb[:-1] <= stim_window_end
        )
        peak_response = np.max(
            np.mean(peth[:, :, stimulus_window_bool], axis=1), axis=1
        )
        peak_responses.append(peak_response)
...
ax3.errorbar(
    ...
    markersize=8,
    capsize=5,
    label=f"Left trials (n={len(left_trials)})",
)
ax3.errorbar(
    x + 0.1,
    right_means,
    ...
    markersize=8,
    capsize=5,
    label=f"Right trials (n={len(right_trials)})",
)
